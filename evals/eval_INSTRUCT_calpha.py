...
def make_inference_prompt(ca_coords, tokenizer, decimal_places=1):
...
def make_full_prompt(ca_coords, seq, tokenizer, decimal_places=1):
    struct_str = format_coords(ca_coords, decimal_places)
...
def load_base(model_name, tokenizer):
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",               # split visible GPUs
    )
...
def compute_perplexity_for_record(model, tokenizer, r, max_length, decimal_places):
    inference_prompt = make_inference_prompt(r["ca_coords"], tokenizer, decimal_places)
    full_str         = make_full_prompt(r["ca_coords"], r["seq"], tokenizer, decimal_places)
...
def compute_recovery(model, tokenizer, records, max_length, decimal_places):
    ...
    for r in tqdm(records, desc="Eval"):
        ...
        prompt = make_inference_prompt(r["ca_coords"], tokenizer, decimal_places)
        inputs = tokenizer(
...
if __name__ == "__main__":
    ...
    p.add_argument("--out",            default="results/eval_instruct_calpha.json")
