...
def make_full_sequence(tokens_3di, seq):
    spaced_seq = " ".join(list(seq.upper()))
    return f"{make_prompt(tokens_3di)} {spaced_seq}"
...
def compute_perplexity_for_record(model, tokenizer, r, max_length):
    prompt   = make_prompt(r["tokens_3di"])
    full_str = make_full_sequence(r["tokens_3di"], r["seq"])
...
def compute_recovery(model, tokenizer, records, max_length):
    ...
    for r in tqdm(records, desc="Eval"):
        ...
        nll, n_residues = compute_perplexity_for_record(model, tokenizer, r, max_length)
        ppl = math.exp(nll) if not math.isnan(nll) else float("nan")
        print(f"ppl:  {ppl:.3f}  (nll={nll:.4f}, residues={n_residues})")

        examples.append({
            "name": r["name"], "length": len(ref), "pred_len": len(pred),
            "ref": ref, "pred": pred, "recovery": c / len(ref),
            "nll": nll, "perplexity": ppl, "n_residues": n_residues,
        })
...
if __name__ == "__main__":
    ...
    p.add_argument("--out",        default="results/eval_NONinstruct_3di.json")
