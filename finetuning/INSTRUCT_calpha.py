...
SYSTEM_PROMPT = (
    ...
    "encoded as Cα (alpha-carbon) coordinates.\n\n"
    "INPUT FORMAT:\n"
    "You receive a sequence of Cα coordinates, one triplet per residue, "
    ...
    "and rounded to 1 decimal place in Ångströms.\n\n"
    "OUTPUT FORMAT:\n"
...
def make_prompt(ca_coords: list, aa_seq: str = "", tokenizer=None,
                decimal_places: int = 1) -> str:
    struct_str = format_coords(ca_coords, decimal_places)

    messages = [
        {"role": "system",    "content": SYSTEM_PROMPT},
        {"role": "user",      "content": f"{STRUCT_BOS} {struct_str}"},
        {"role": "assistant", "content": f"{SEQ_BOS} {' '.join(list(aa_seq.upper()))}" if aa_seq else SEQ_BOS},
    ]
...
def make_inference_prompt(ca_coords: list, tokenizer, decimal_places: int = 1) -> str:
    struct_str = format_coords(ca_coords, decimal_places)
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user",   "content": f"{STRUCT_BOS} {struct_str}"},
...
class InverseFoldingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_prompt = make_prompt(
            r["ca_coords"], r["seq"], self.tokenizer, self.decimal_places,
        )
        ...
        if seq_bos_id in input_ids:
            ...
            raise ValueError(
                f"<seq> token (id={seq_bos_id}) not found in tokenized prompt for record {idx}. "
                f"Check that the chat template is including the assistant turn correctly."
            )
...
def main(args):
    ...
    json.dump({
        ...
        "lr":             args.lr,
        "epochs":         args.epochs,
        "prompt_format":  "instruct_chat_template_calpha_raw_floats",
...
if __name__ == "__main__":
    ...
    p.add_argument("--out_dir",        default="results/llm_calpha_instruct_v1")
